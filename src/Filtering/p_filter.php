 ...
 * ```
 * [1, 2, 3, 4, 5]
 * |> p_filter_preserving_keys(static fn (int $x) => $x % 2 === 0);
 * //= [1 => 2, 3 => 4]
 * ```
 * Filter elements with keys shorter than three characters
 * ```
 * ['a' => 1, 'bb' => 2, 'ccc' => 3]
 * |> p_filter_preserving_keys(static fn (int $x, string $k) => strlen($k) < 3);
 * //= ['a' => 1, 'bb' => 2]
 * ```
 *
 * @template K
 ...
 */
function p_filter_preserving_keys(callable $predicate): callable
{
    return static fn (iterable $iterable): iterable => new LazyRewindableIterator(static function () use ($iterable, $predicate): iterable {
        foreach ($iterable as $key => $value) {
            if ($predicate($value, $key)) {
